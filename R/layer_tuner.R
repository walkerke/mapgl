...
parse_pipeline_calls <- function(call) {
  ...
  if (
    ...
    for (i in seq_along(args)) {
      name <- if (!is.null(arg_names) && arg_names[i] != "") {
        arg_names[i]
      } else {
        ""
      }
    ...
    if (fun %in% c("|>", "%>%")) {
      ...
      if (is.call(rhs)) {
        ...
        for (i in seq_along(rhs_args)) {
          name <- if (!is.null(rhs_arg_names) && rhs_arg_names[i] != "") {
            rhs_arg_names[i]
          } else {
            ""
          }
...
add_layer_tuner <- function(
  ...
  tryCatch(
    {
      ...
      if (!is.null(pipeline_call)) {
        ...
        original_calls <- Filter(
          function(x) x$fun != "add_layer_tuner",
          original_calls
        )
      }
    },
  ...
  if (
    inherits(map, "mapboxgl_compare") || inherits(map, "maplibregl_compare")
  ) {
  ...
  layer_tuner_dep <- htmltools::htmlDependency(
    ...
    version = "1.0.0",
    src = c(
      file = system.file("htmlwidgets/lib/layer-tuner", package = "mapgl")
    ),
  ...
  if (is.numeric(value)) {
    if (
      ...
      rlang::abort(paste0(
        "`",
        arg,
        "` must be a positive number or CSS length."
      ))
    }
