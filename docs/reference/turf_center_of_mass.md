...
This function calculates the center of mass (geometric centroid) for
each feature. Uses turf.centerOfMass which provides more accurate
centroids than turf.centroid, matching the behavior of sf::st_centroid()
and PostGIS ST_Centroid. The result is added as a source to the map,
which can then be styled using add_circle_layer(), etc.
...
- layer_id:
  ...
  The ID of a layer or source (mutually exclusive with data and
  coordinates).
...
- source_id:
  ...
  The ID for the new source containing the center of mass points.
  Required.
