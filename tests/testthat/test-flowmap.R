...
test_that("add_flowmap serializes tooltip content and style", {
  ...
  tt <- default_tooltip$x$flowmaps[[1]]$tooltip
  expect_true(tt$enabled)
  expect_true(tt$location)
  expect_true(tt$flow)
  # Default style is the light preset (the test map is not dark).
  expect_equal(tt$style$background_color, "rgba(255, 255, 255, 0.96)")
  ...
  expect_equal(
    dark_tooltip$x$flowmaps[[1]]$tooltip$style$background_color,
    "rgba(35, 35, 35, 0.94)"
  )

  # Per-type content: a {brace} location template + a concat() flow expression,
  # with an explicit dark anchored style.
  custom <- flowmap_test_map(
    tooltip = list(
      location = "<strong>{name}</strong>",
      flow = concat(get_column("origin_name"), " -> ", get_column("dest_name"))
    ),
    tooltip_style = tooltip_style("dark", position = "anchored")
  )
  tt <- custom$x$flowmaps[[1]]$tooltip
  expect_equal(tt$location, "<strong>{name}</strong>")
  expect_true(is.list(tt$flow) && identical(tt$flow[[1]], "concat"))
  expect_equal(tt$style$position, "anchored")
  expect_equal(tt$style$background_color, "rgba(35, 35, 35, 0.94)")

  # A single string applies to both object types.
  same <- flowmap_test_map(tooltip = "{count}")
  expect_equal(same$x$flowmaps[[1]]$tooltip$location, "{count}")
  expect_equal(same$x$flowmaps[[1]]$tooltip$flow, "{count}")

  # Selectively disable one object type.
...
test_that("add_flowmap validates tooltip content and style", {
  expect_error(
  ...
  expect_error(
    ...
    "must be TRUE, FALSE, a column name"
  )
})
...
test_that("set_flowmap_filter validates its arguments", {
  flowmap_proxy <- function() {
    structure(
      list(id = "map", session = list(sendCustomMessage = function(...) NULL)),
      class = "maplibre_proxy"
    )
  }

  expect_error(
    set_flowmap_filter(flowmap_proxy(), id = ""),
    "non-empty character"
  )
  expect_error(
    set_flowmap_filter(flowmap_proxy(), "flows", selected_locations = 1:3),
    "character vector"
  )
  expect_error(
    set_flowmap_filter(
  ...
  expect_error(
    set_flowmap_filter(
      flowmap_proxy(),
      "flows",
      selected_time_range = c("a", "b")
    ),
    "Date.*POSIXct.*numeric|numeric"
  )
})

test_that("set_flowmap_filter accepts valid inputs and sends a message", {
  messages <- list()
  session <- list(
    sendCustomMessage = function(type, message) {
      messages[[length(messages) + 1]] <<- list(type = type, message = message)
  ...
  result <- set_flowmap_filter(
    proxy,
    "flows",
    selected_locations = c("a", "b"),
    location_filter_mode = "INCOMING",
    selected_time_range = as.Date(c("2024-01-01", "2024-01-31"))
  )

  expect_identical(result, proxy)
  expect_length(messages, 1)
  expect_equal(messages[[1]]$message$message$type, "set_flowmap_filter")
  expect_equal(messages[[1]]$message$message$id, "flows")
  expect_equal(
    messages[[1]]$message$message$filter$locationFilterMode,
    "INCOMING"
  )
  expect_equal(
    messages[[1]]$message$message$filter$selectedLocations,
    c("a", "b")
  )
})
...
test_that("flowmap dependencies are attached on demand, not in the widget YAML", {
  # The flowmap libraries are heavy (~1.6MB), so they must NOT be in the
  # always-on widget YAML. A map that never adds a flowmap should not load them.
  ...
  expect_false(any(grepl("flowmap", mapbox)))
  expect_false(any(grepl("flowmap", maplibre)))

  # A plain map carries no flowmap dependencies.
  plain_deps <- vapply(maplibre()$dependencies, function(d) d$name, character(1))
  expect_false("flowmap-plugin" %in% plain_deps)
  expect_false("flowmap-gl" %in% plain_deps)

  # add_flowmap() attaches flowmap-gl, flowmap-plugin, and d3 on demand.
  map <- flowmap_test_map()
  dep_names <- vapply(map$dependencies, function(d) d$name, character(1))
  expect_true("flowmap-gl" %in% dep_names)
  expect_true("flowmap-plugin" %in% dep_names)
  expect_true("d3" %in% dep_names)

  # Adding a second flowmap does not duplicate the dependencies.
  map2 <- add_flowmap(
    map,
    id = "flows-2",
    locations = flowmap_test_locations(),
  ...
  dep_names2 <- vapply(map2$dependencies, function(d) d$name, character(1))
  expect_equal(sum(dep_names2 == "flowmap-plugin"), 1)
})

test_that("compare widgets include and initialize flowmap support", {
  # Flowmap dependencies are not baked into the compare YAML; they ride on each
  # map's dependencies and are propagated by compare().
  ...
  for (path in compare_yaml_paths) {
    ...
    expect_false(any(grepl("flowmap", lines)), info = path)
  }

  # compare() propagates a flowmap map's dependencies into the compare widget.
  widget <- compare(flowmap_test_map(), maplibre())
  widget_deps <- vapply(widget$dependencies, function(d) d$name, character(1))
  expect_true("flowmap-gl" %in% widget_deps)
  expect_true("flowmap-plugin" %in% widget_deps)
...
test_that("flowmap plugin includes tooltip renderers", {
  ...
  expect_match(js, 'DEFAULT_FLOW_TOOLTIP = "<strong>{count}</strong> trips"', fixed = TRUE)
...
test_that("is_dark_style utility classifies basemaps correctly", {
  ...
  # Fallbacks default to light: dark mode is only assumed when a style
  # explicitly looks dark, so the default (light) Standard basemap is not
  # mislabeled as dark.
  expect_false(is_dark_style(NULL))
  expect_false(is_dark_style(list()))
  expect_false(is_dark_style("random-style"))
})
