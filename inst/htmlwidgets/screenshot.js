...
function applyMapScreenshotOptions(map, options) {
  ...
  if (options.hide_controls) {
    ...
    container.querySelectorAll('.layers-control, .mapgl-measurement-box, .mapgl-coordinates-control, .maplibregl-ctrl-geocoder, .mapboxgl-ctrl-geocoder, .maptiler-ctrl, .lil-gui').forEach(el => {
      hiddenElements.push({ element: el, display: el.style.display });
      el.style.display = 'none';
    });
  }
