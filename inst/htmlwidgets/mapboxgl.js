    ...
    return {
      renderValue: function (x) {
        ...
        map.on("style.load", function () {
          ...
          // Add box query control if enabled
          if (x.box_query_control) {
            const boxQueryOptions = x.box_query_control;
            
            class BoxQueryControl {
              constructor(options) {
                ...
                // Pre-bind event handlers to maintain proper references for cleanup
              ...
              onAdd(map) {
                ...
                this.button.className = 'mapboxgl-ctrl-icon mapboxgl-ctrl-box-query';
                this.button.type = 'button';
                this.button.setAttribute('aria-label', 'Box Query');
                this.button.title = 'Box Query: Hold Shift+Click to draw selection box';
                
                // Add icon styling - selection box with dashed corners
                this.button.innerHTML = `
                  <svg width="20" height="20" viewBox="0 0 20 20" style="vertical-align: top;">
                    <!-- Top-left corner -->
                    <path d="M3 3 L3 7 M3 3 L7 3" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round"/>
                    <!-- Top-right corner -->
                    <path d="M17 3 L17 7 M17 3 L13 3" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round"/>
                    <!-- Bottom-left corner -->
                    <path d="M3 17 L3 13 M3 17 L7 17" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round"/>
                    <!-- Bottom-right corner -->
                    <path d="M17 17 L17 13 M17 17 L13 17" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round"/>
                    <!-- Side handles -->
                    <rect x="9" y="2" width="2" height="2" fill="currentColor" rx="0.5"/>
                    <rect x="2" y="9" width="2" height="2" fill="currentColor" rx="0.5"/>
                    <rect x="16" y="9" width="2" height="2" fill="currentColor" rx="0.5"/>
                    <rect x="9" y="16" width="2" height="2" fill="currentColor" rx="0.5"/>
                  </svg>
                `;
                ...
                // Add CSS for box drawing
                if (!document.getElementById('mapgl-box-query-css')) {
                  const style = document.createElement('style');
                  style.id = 'mapgl-box-query-css';
                  style.textContent = `
                    .mapgl-boxdraw {
                      background: rgba(${this.hexToRgb(boxQueryOptions.box_color || '#3887be')}, ${boxQueryOptions.box_opacity || 0.1});
                      border: ${boxQueryOptions.box_border_width || 2}px solid ${boxQueryOptions.box_border_color || '#3887be'};
                      position: absolute;
                      top: 0;
                      left: 0;
                      width: 0;
                    ...
                    .mapboxgl-ctrl-box-query.active {
              ...
              activate() {
                ...
                // Add event listener for mousedown - always listening when active
              ...
              mousePos(e) {
                ...
                return {
                  x: e.clientX - rect.left - this.canvas.clientLeft,
                  y: e.clientY - rect.top - this.canvas.clientTop
                };
              }

              mouseDown(e) {
                // Only proceed if Shift key is held and left mouse button
              ...
              onMouseMove(e) {
                // Only continue if Shift key is still being held
                if (!e.shiftKey) {
                  this.finish();
                  return;
                }
                ...
                if (!this.box) {
                  ...
                  this.box.classList.add('mapgl-boxdraw');
              ...
              onKeyUp(e) {
                // Cancel drawing if Shift key is released
              ...
              finish(bbox) {
                ...
                // Always clear highlighted layer first if no bbox (cancelled selection)
                if (!bbox && boxQueryOptions.highlighted_layer) {
                  this.map.setFilter(boxQueryOptions.highlighted_layer, ['in', 'id', '']);
                }

                // If bbox exists, perform query
                if (bbox && bbox.length === 2) {
                  // Convert point array to proper bbox format
                  const point1 = bbox[0];
                  const point2 = bbox[1];
                  const bboxArray = [[point1.x, point1.y], [point2.x, point2.y]];
                  
                  let queryOptions = {};
                  if (boxQueryOptions.layers) {
                    queryOptions.layers = Array.isArray(boxQueryOptions.layers) ? 
                      boxQueryOptions.layers : [boxQueryOptions.layers];
                  }

                  const features = this.map.queryRenderedFeatures(bboxArray, queryOptions);

                  // Check feature limit
                  if (features.length >= boxQueryOptions.max_features) {
                    alert(`Select a smaller number of features (found ${features.length}, max ${boxQueryOptions.max_features})`);
                    this.map.dragPan.enable();
                    return;
                  }
                  ...
                  // Update highlighted layer if specified
                  if (boxQueryOptions.highlighted_layer) {
                    if (deduplicatedFeatures.length > 0) {
                      // Determine filter property and extract identifiers
                      let filterProperty = boxQueryOptions.filter_property;
                      let featureIds;
                      
                      if (filterProperty) {
                        // Use specified property
                        featureIds = deduplicatedFeatures.map(feature => 
                          feature.properties[filterProperty]
                        ).filter(id => id !== undefined);
                      } else {
                        // Try feature.id first, then first available property
                        featureIds = deduplicatedFeatures.map(feature => feature.id).filter(id => id !== undefined);
                        
                        if (featureIds.length === 0 && deduplicatedFeatures.length > 0) {
                          // Use first available property as fallback
                          const firstFeature = deduplicatedFeatures[0];
                          const propertyKeys = Object.keys(firstFeature.properties);
                          if (propertyKeys.length > 0) {
                            filterProperty = propertyKeys[0];
                            featureIds = deduplicatedFeatures.map(feature => 
                              feature.properties[filterProperty]
                            ).filter(id => id !== undefined);
                          }
                      ...
                      if (featureIds && featureIds.length > 0 && filterProperty) {
                        // Apply filter to highlighted layer - build filter expression without spread operator
                        const filterExpression = ['in', filterProperty].concat(featureIds);
                        this.map.setFilter(boxQueryOptions.highlighted_layer, filterExpression);
                      }
                    } else {
                      // Clear highlighted layer when no features selected
                      this.map.setFilter(boxQueryOptions.highlighted_layer, ['in', 'id', '']);
                    }
                  }

                  // Send results to Shiny if available
            ...
            const boxQueryControl = new BoxQueryControl(boxQueryOptions);
            map.addControl(boxQueryControl, boxQueryOptions.position || "top-right");
            map.controls.push(boxQueryControl);
          }
  ...
  Shiny.addCustomMessageHandler("mapboxgl-proxy", function (data) {
    ...
    if (map) {
      ...
      if (message.type === "set_filter") {
        ...
        map.controls.push(customControl);
      } else if (message.type === "add_box_query_control") {
        // Create box query control
        const boxQueryOptions = message.options;
        ...
        class BoxQueryControl {
          ...
          deactivate() {
            ...
            // Remove event listeners
            this.canvas.removeEventListener('mousedown', this.mouseDown.bind(this), true);
          ...
          finish(bbox) {
            ...
            if (bbox && bbox.length === 2) {
              ...
              if (features && features.length > 0) {
                ...
                const filterProperty = boxQueryOptions.filter_property || 'id';
                const featureIds = [...new Set(
                  features.map(f => f.properties[filterProperty])
                    .filter(id => id !== undefined && id !== null)
                )];
                
                if (featureIds && featureIds.length > 0) {
                  // Apply filter to highlighted layer
                  const filterExpression = ['in', filterProperty].concat(featureIds);
                  this.map.setFilter(boxQueryOptions.highlighted_layer, filterExpression);
                }
                ...
                if (typeof Shiny !== 'undefined') {
                  ...
                  Shiny.setInputValue(this.map.getContainer().id + '_box_query_features', JSON.stringify(featureCollection));
                }
              }
            }
            ...
            if (bbox) {
              ...
              const features = this.map.queryRenderedFeatures(bbox, queryOptions);
